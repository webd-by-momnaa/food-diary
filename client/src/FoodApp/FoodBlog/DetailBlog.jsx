...
import FoodNavSix from '../AllFoodNavbars/FoodNavSix'
    ...
    <div className="blog-container">
      {card?.map((v, i) => (
        <div className="blog-card" key={v._id || i}>
          <div className="mt- mx-12 mb-5">
            {/* Title and Subtitle */}
            <div className="text-center text-sm text-slate-700 font-mono"><b>Blog / </b>{v.title}</div>
            <b className="text-3xl">{v.title || "Untitled Blog"}</b>
            <div className="text-lg">{v.subTitle || "No subtitle available"}</div>

            {/* Image */}
            <div className="image-container w-full h-full object-cover object-center my-5">
              <img
